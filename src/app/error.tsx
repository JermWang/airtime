...
export default function ErrorPage({ error, retry }: { error: Error & { digest?: string }; retry: () => void }) {
  return (
    <main className="mx-auto flex min-h-dvh max-w-xl flex-col justify-center gap-5 px-6">
      <p className="readout text-xs uppercase tracking-widest text-signal">AIRTIME · Connection interrupted</p>
      ...
      {error.digest && <p className="readout text-xs text-ink-400">Error reference: {error.digest}</p>}
    </main>
  );
}
