...
const SECTIONS = [
  ...
  { id: "token-rewards", title: "Token and rewards" },
    ...
    <PageFrame wide>
      ...
      <div className="mt-12 grid gap-12 md:grid-cols-[210px_minmax(0,1fr)] md:gap-14">
        ...
        <div className="flex min-w-0 flex-col gap-10">
          ...
          <Section id="token-rewards" title="Token and rewards">
            <P>Two separate streams of money run through this network, and they do different jobs. Advertising revenue retires supply. Trading fees pay holders. Neither one funds the other.</P>
            <P><strong className="text-signal">Launch policy — pending activation.</strong> AIRTIME&apos;s token mint and the automated buyback, burn, snapshot and distribution services are not active yet. The rules below describe the intended operation once activated; no rewards are currently promised or accruing under this policy.</P>
            <h3 className="text-lg font-medium text-ink-50">Where each stream goes</h3>
            <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b border-white/15"><th className="p-3">Stream</th><th className="p-3">Source</th><th className="p-3">Destination</th></tr></thead><tbody className="text-ink-300"><tr className="border-b border-white/10"><td className="p-3">Advertising revenue</td><td className="p-3">Every surface sold on the station, settled in SOL</td><td className="p-3">Buys AIRTIME on the open market and burns it</td></tr><tr><td className="p-3">Trading fees</td><td className="p-3">Creator fees on AIRTIME trades at StonkFun</td><td className="p-3">Buys Anduril pre-stock exposure for daily allocation to eligible holders</td></tr></tbody></table></div>
            <P>Advertising revenue is the network&apos;s business income, so it is designated for reducing the number of tokens that exist. A completed burn permanently destroys tokens and has a public transaction record.</P>
            <P>Trading fees are generated by the market rather than by the business, so they are designated for holder rewards in the asset the treasury is mandated to hold.</P>
            <h3 className="text-lg font-medium text-ink-50">Daily pre-stock rewards</h3>
            <P>Creator fees accrued at StonkFun are scheduled to be swept daily at 00:00 UTC and used to acquire Anduril pre-stock exposure. Eligible balances are determined from the last finalized Solana slot before 00:00 UTC. The first sweep follows the first complete UTC day after activation.</P>
            <Bullets items={[
              <>Eligibility requires at least 1 AIRTIME at the snapshot. Allocation is proportional to each eligible wallet&apos;s balance relative to the total eligible balance.</>,
              <>Liquidity pools, the treasury wallet, burn addresses and all operator-held wallets are excluded. Those balances never earn rewards. The exclusion list must be published before activation.</>,
              <>Rewards are intended to accrue to each eligible wallet and be distributed automatically, without a claim transaction. The custody and distribution mechanism must be published before activation.</>,
              <>Daily allocation depends on fees actually collected and pre-stock exposure actually acquired. If acquisition or settlement is delayed, the funds carry forward; there is no guaranteed daily payout.</>,
            ]} />
            <h3 className="text-lg font-medium text-ink-50">Buyback and burn</h3>
            <P>Advertising revenue is scheduled for a separate daily sweep at 00:00 UTC, after retaining funds needed for pending refunds. It is designated for buying AIRTIME on the open market and permanently burning the acquired tokens with the Solana token program.</P>
            <P>Completed buybacks and burns must have on-chain transaction references listed on the treasury page. An operator-entered record alone is not proof of execution. If a swap cannot execute within its configured limits, the funds remain earmarked for a later sweep.</P>
            <P>Burned tokens cannot be recovered. Burns reduce supply; a claim that supply can never increase also requires the token&apos;s mint authority to be revoked and verified at launch.</P>
          </Section>
